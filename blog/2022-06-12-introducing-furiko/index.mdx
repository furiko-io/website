...
We were constantly tuning system and JVM parameters, but eventually found ourselves hitting a ceiling with regards to usability, reliability and performance. At one point, the Rundeck platform crashed every other week, which severely affected our users' trust in the platform when their jobs did not run, or was stuck indefinitely due to an _Incomplete_ state. 😭
...
1. We wanted to automatically kill the job if it does not start running within a timeout, for example if the container image no longer exists.
2. We need to ensure that executing ad-hoc jobs would not violate the concurrency policy of the parent CronJob.
3. We cannot guarantee that workloads will not be executed more than once, such as Pod deletion or eviction by kube-controller-manager. This was especially important to preventing double-write issues, for example when performing financial transactions.
4. We wanted to support custom cron syntaxes, such as the `H` format [found in Jenkins](https://www.jenkins.io/doc/book/pipeline/syntax/#cron-syntax).
5. At the time, [CronJobControllerV2](https://github.com/kubernetes/kubernetes/pull/93370) did not yet exist, and several performance problems riddled the V1 controller. Because it did not use the informer pattern, Jobs were not scheduled timely and the controller could not scale to reconcile thousands of Jobs concurrently.
...
To add on, we were also looking to support features that were already existing in Rundeck, in order to migrate our existing customers to the new Kubernetes platform. These features included but were not limited to:

1. Parameterizing jobs, allowing us to reuse them as templates.
2. Schedule a job for later, rather than executing it immediately.
3. Enqueueing a bunch of jobs to be run after the previous one finishes, such that they will not run concurrently.
...
Check out Furiko on [GitHub](https://github.com/furiko-io/furiko), or the [installation guide](/docs/getting-started/install) to get started on a Kubernetes cluster in 1 minute.
